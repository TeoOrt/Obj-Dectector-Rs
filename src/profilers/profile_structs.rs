...
impl HrtProfiler {
    pub fn new()->Self{
        HrtProfiler { start: Instant::now(), stop: Instant::now() , time_stamps: Vec::new() }
    }
}
...
impl Profile for HrtProfiler {
    ...
    fn get_stats(self) -> ProfileStats {
        ...
        ProfileStats { avg_stops: avg.div_ceil(elapsed_times.len() as u128) , min_stop: min.as_micros(), max_stop: max.as_micros() }
    }
}
